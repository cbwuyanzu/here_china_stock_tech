...
#if 0
void testConsumePopBeforeProduceFuc(int &timeoutCnt, int &wakeupCnt, int &tryCnt) {
    MyFifoQueue<RawSzMDData> q;
    volatile int doneFlag = 0;
    std::thread consumerThread(consumerThreadFuncPop, std::ref(q),0,0, std::ref(doneFlag),std::ref(tryCnt),std::ref(timeoutCnt),std::ref(wakeupCnt));
    std::thread producerThread(producerThreadFunc, std::ref(q), PreSleepMs,0, std::ref(doneFlag));
    consumerThread.join();
    producerThread.join();
    assert(q.empty());
}

void testConsumePopWhileProduceFuc(int &timeoutCnt, int &wakeupCnt, int &tryCnt) {
    MyFifoQueue<RawSzMDData> q;
    volatile int doneFlag = 0;
    std::thread consumerThread(consumerThreadFuncPop, std::ref(q),0,0,std::ref(doneFlag),std::ref(tryCnt),std::ref(timeoutCnt),std::ref(wakeupCnt));
    std::thread producerThread(producerThreadFunc, std::ref(q), 0,0,std::ref(doneFlag));
    consumerThread.join();
    producerThread.join();
    assert(q.empty());
}
void testConsumePopAfterProduceFuc(int &timeoutCnt, int &wakeupCnt, int &tryCnt) {
    MyFifoQueue<RawSzMDData> q;
    volatile int doneFlag = 0;
    std::thread consumerThread(consumerThreadFuncPop, std::ref(q),PreSleepMs,0,std::ref(doneFlag),std::ref(tryCnt),std::ref(timeoutCnt),std::ref(wakeupCnt));
    std::thread producerThread(producerThreadFunc, std::ref(q), 0,0,std::ref(doneFlag));
    consumerThread.join();
    producerThread.join();
    assert(q.empty());
...
void testConsumeTryPopVPBeforeProduceFuc(int &timeoutCnt, int &wakeupCnt, int &tryCnt) {
    MyFifoQueue<RawSzMDData> q;
    volatile int doneFlag = 0;
    std::thread consumerThread(consumerThreadFuncTryPopVP, std::ref(q),0,LoopSleepMs,std::ref(doneFlag),std::ref(tryCnt),std::ref(timeoutCnt),std::ref(wakeupCnt));
    std::thread producerThread(producerThreadFunc, std::ref(q), PreSleepMs,0,std::ref(doneFlag));
    consumerThread.join();
    producerThread.join();
    assert(q.empty());
...
void testConsumeTryPopVPWhileProduceFuc(int &timeoutCnt, int &wakeupCnt, int &tryCnt) {
    MyFifoQueue<RawSzMDData> q;
    volatile int doneFlag = 0;
    std::thread consumerThread(consumerThreadFuncTryPopVP, std::ref(q),0,LoopSleepMs,std::ref(doneFlag),std::ref(tryCnt),std::ref(timeoutCnt),std::ref(wakeupCnt));
    std::thread producerThread(producerThreadFunc, std::ref(q), 0,0,std::ref(doneFlag));
...
void testConsumeTryPopVPAfterProduceFuc(int &timeoutCnt, int &wakeupCnt, int &tryCnt) {
    MyFifoQueue<RawSzMDData> q;
    volatile int doneFlag = 0;
    std::thread consumerThread(consumerThreadFuncTryPopVP, std::ref(q),PreSleepMs,LoopSleepMs,std::ref(doneFlag),std::ref(tryCnt),std::ref(timeoutCnt),std::ref(wakeupCnt));
    std::thread producerThread(producerThreadFunc, std::ref(q), 0,0,std::ref(doneFlag));
    consumerThread.join();
    producerThread.join();
    assert(q.empty());
...
void testConsumeTryPopVNPBeforeProduceFuc(int &timeoutCnt, int &wakeupCnt, int &tryCnt) {
    MyFifoQueue<RawSzMDData> q;
    volatile int doneFlag = 0;
    std::thread consumerThread(consumerThreadFuncTryPopVNP, std::ref(q),0,LoopSleepMs,std::ref(doneFlag),std::ref(tryCnt),std::ref(timeoutCnt),std::ref(wakeupCnt));
    std::thread producerThread(producerThreadFunc, std::ref(q), PreSleepMs,0,std::ref(doneFlag));
    consumerThread.join();
    producerThread.join();
    assert(q.empty());
...
void testConsumeTryPopVNPWhileProduceFuc(int &timeoutCnt, int &wakeupCnt, int &tryCnt) {
    MyFifoQueue<RawSzMDData> q;
    volatile int doneFlag = 0;
    std::thread consumerThread(consumerThreadFuncTryPopVNP, std::ref(q),0,LoopSleepMs,std::ref(doneFlag),std::ref(tryCnt),std::ref(timeoutCnt),std::ref(wakeupCnt));
    std::thread producerThread(producerThreadFunc, std::ref(q), 0,0,std::ref(doneFlag));
...
void testConsumeTryPopVNPAfterProduceFuc(int &timeoutCnt, int &wakeupCnt, int &tryCnt) {
    MyFifoQueue<RawSzMDData> q;
    volatile int doneFlag = 0;
    std::thread consumerThread(consumerThreadFuncTryPopVNP, std::ref(q),PreSleepMs,LoopSleepMs,std::ref(doneFlag),std::ref(tryCnt),std::ref(timeoutCnt),std::ref(wakeupCnt));
    std::thread producerThread(producerThreadFunc, std::ref(q), 0,0,std::ref(doneFlag));
    consumerThread.join();
    producerThread.join();
    assert(q.empty());
...
void testConsumeTryPopPBeforeProduceFuc(int &timeoutCnt, int &wakeupCnt, int &tryCnt) {
    MyFifoQueue<RawSzMDData> q;
    volatile int doneFlag = 0;
    std::thread consumerThread(consumerThreadFuncTryPopP, std::ref(q),0,LoopSleepMs,std::ref(doneFlag),std::ref(tryCnt),std::ref(timeoutCnt),std::ref(wakeupCnt));
    std::thread producerThread(producerThreadFunc, std::ref(q), PreSleepMs,0,std::ref(doneFlag));
    consumerThread.join();
    producerThread.join();
    assert(q.empty());
...
void testConsumeTryPopPWhileProduceFuc(int &timeoutCnt, int &wakeupCnt, int &tryCnt) {
    MyFifoQueue<RawSzMDData> q;
    volatile int doneFlag = 0;
    std::thread consumerThread(consumerThreadFuncTryPopP, std::ref(q),0,LoopSleepMs,std::ref(doneFlag),std::ref(tryCnt),std::ref(timeoutCnt),std::ref(wakeupCnt));
    std::thread producerThread(producerThreadFunc, std::ref(q), 0,0,std::ref(doneFlag));
...
void testConsumeTryPopPAfterProduceFuc(int &timeoutCnt, int &wakeupCnt, int &tryCnt) {
    MyFifoQueue<RawSzMDData> q;
    volatile int doneFlag = 0;
    std::thread consumerThread(consumerThreadFuncTryPopP, std::ref(q),PreSleepMs,LoopSleepMs,std::ref(doneFlag),std::ref(tryCnt),std::ref(timeoutCnt),std::ref(wakeupCnt));
    std::thread producerThread(producerThreadFunc, std::ref(q), 0,0,std::ref(doneFlag));
    consumerThread.join();
    producerThread.join();
    assert(q.empty());
...
void testConsumeTryPopNPBeforeProduceFuc(int &timeoutCnt, int &wakeupCnt, int &tryCnt) {
    MyFifoQueue<RawSzMDData> q;
    volatile int doneFlag = 0;
    std::thread consumerThread(consumerThreadFuncTryPopNP, std::ref(q),0,LoopSleepMs,std::ref(doneFlag),std::ref(tryCnt),std::ref(timeoutCnt),std::ref(wakeupCnt));
    std::thread producerThread(producerThreadFunc, std::ref(q), PreSleepMs,0,std::ref(doneFlag));
    consumerThread.join();
    producerThread.join();
    assert(q.empty());
...
void testConsumeTryPopNPWhileProduceFuc(int &timeoutCnt, int &wakeupCnt, int &tryCnt) {
    MyFifoQueue<RawSzMDData> q;
    volatile int doneFlag = 0;
    std::thread consumerThread(consumerThreadFuncTryPopNP, std::ref(q),0,LoopSleepMs,std::ref(doneFlag),std::ref(tryCnt),std::ref(timeoutCnt),std::ref(wakeupCnt));
    std::thread producerThread(producerThreadFunc, std::ref(q), 0,0,std::ref(doneFlag));
...
void testConsumeTryPopNPAfterProduceFuc(int &timeoutCnt, int &wakeupCnt, int &tryCnt) {
    MyFifoQueue<RawSzMDData> q;
    volatile int doneFlag = 0;
    std::thread consumerThread(consumerThreadFuncTryPopNP, std::ref(q),PreSleepMs,LoopSleepMs,std::ref(doneFlag),std::ref(tryCnt),std::ref(timeoutCnt),std::ref(wakeupCnt));
    std::thread producerThread(producerThreadFunc, std::ref(q), 0,0,std::ref(doneFlag));
    consumerThread.join();
    producerThread.join();
    assert(q.empty());
...
int main() {
    ...
    return 0;
#if 0
    console_logger->info("QueueTest Consume BEFORE Produce start! numLoops {}", NumLoops);
    runTest(testConsumePopBeforeProduceFuc, "testConsumePopBeforeProduceFuc");
    runTest(testConsumeTryPopVPBeforeProduceFuc, "testConsumeTryPopVPBeforeProduceFuc");
    runTest(testConsumeTryPopVNPBeforeProduceFuc, "testConsumeTryPopVNPBeforeProduceFuc");
    runTest(testConsumeTryPopPBeforeProduceFuc, "testConsumeTryPopPBeforeProduceFuc");
    runTest(testConsumeTryPopNPBeforeProduceFuc, "testConsumeTryPopNPBeforeProduceFuc");
    console_logger->info("QueueTest Consume WHILE Produce start! numLoops {}", NumLoops);
    runTest(testConsumePopWhileProduceFuc, "testConsumePopWhileProduceFuc");
    runTest(testConsumeTryPopVPWhileProduceFuc, "testConsumeTryPopVPWhileProduceFuc");
    runTest(testConsumeTryPopVNPWhileProduceFuc, "testConsumeTryPopVNPWhileProduceFuc");
    runTest(testConsumeTryPopPWhileProduceFuc, "testConsumeTryPopPWhileProduceFuc");
    runTest(testConsumeTryPopNPWhileProduceFuc, "testConsumeTryPopNPWhileProduceFuc");
    console_logger->info("QueueTest Consume AFTER Produce start! numLoops {}", NumLoops);
    runTest(testConsumePopAfterProduceFuc, "testConsumePopAfterProduceFuc");
    runTest(testConsumeTryPopVPAfterProduceFuc, "testConsumeTryPopVPAfterProduceFuc");
    runTest(testConsumeTryPopVNPAfterProduceFuc, "testConsumeTryPopVNPAfterProduceFuc");
    runTest(testConsumeTryPopPAfterProduceFuc, "testConsumeTryPopPAfterProduceFuc");
    runTest(testConsumeTryPopNPAfterProduceFuc, "testConsumeTryPopNPAfterProduceFuc");
#endif
}
